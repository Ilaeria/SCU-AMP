...
<div id="wrapper">
    ...
    <?php include 'html_includes/header_nav.inc'; ?>
  	...
  	<main role="main">
		<?php
			//If Author has pressed Submit, show editable Profile`
			if(isset($_POST["submit"]))
			{
				//GetProfileID
				$_SESSION["ProfileID"]=$_POST["ProfileID"];
				include("php_includes/editProfile.php");
			}// end if
			else
			//If Author has NOT pressed Submit yet - prompt user to select a Profile to display
			{
				...
				//Display Profile(s)
				{
					...
					switch($numProfiles)
					{
						//If Author has 1 Profile => Show Editable Profile
